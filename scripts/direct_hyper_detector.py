...
def analyze_session(
    ...
    for round_number in range(1, args.rounds + 1):
        ...
        if reached_100 is None and capital >= 100:
            reached_100 = round_number

        if reached_250 is None and capital >= 250:
            reached_250 = round_number

        if reached_500 is None and capital >= 500:
            reached_500 = round_number

        if reached_750 is None and capital >= 750:
            reached_750 = round_number

        if reached_1000 is None and capital >= 1000:
            reached_1000 = round_number
    ...
    return {
        ...
        "rounds_before_1k": rounds_before_1k,
...
def print_group(
    ...
    print(
        ...
        f"avg_r100={round(mean(r['r100'] for r in rows if r['r100'] is not None),2)} | "
        f"avg_r250={round(mean(r['r250'] for r in rows if r['r250'] is not None),2)} | "
        f"avg_r500={round(mean(r['r500'] for r in rows if r['r500'] is not None),2)} | "
        f"avg_r1000={round(mean(r['r1000'] for r in rows if r['r1000'] is not None),2)}"
    )
