...
def run_protected_session(
    ...
    for round_number in range(1, rounds + 1):
        ...
        if not protected_once and active >= protect_from and protect_ratio > 0:
        ...
        if hasattr(agent, "observe"):
            agent.observe(result.won, active)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run direct sessions with internal capital protection")
    ...
    parser.add_argument("--protect-from", type=float, default=25000.0)
    parser.add_argument("--protect-ratio", type=float, default=0.30)
    ...
    protected_count = sum(1 for row in rows if row["protected_once"])
    objective_count = sum(1 for row in rows if row["final_total"] >= args.target_capital)
    ...
    print(f"Objective rate: {round((objective_count / args.sessions) * 100, 6)} %")
    print(f"Protection reached rate: {round((protected_count / args.sessions) * 100, 6)} %")
    print(f"Best final total: {best_row['final_total']} €")
    print(f"Best session index: {best_row['session_index']}")
    print(f"Best protected amount: {best_row['protected']} €")
    print(f"Best reason: {best_row['reason']}")

    if args.export_csv:
        output_dir = Path("results")
        output_dir.mkdir(exist_ok=True)
        output_path = output_dir / f"protected_session_{args.strategy}.csv"
