...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Qualify a large deterministic sample and keep the strongest subset"
    )
    ...
    for local_index in range(args.source_sessions):
        ...
        if (local_index + 1) % 10000 == 0:
            print(
                f"Scanned "
                f"{local_index + 1}/"
                f"{args.source_sessions} sessions"
            )
    ...
    objective_count = sum(
        1
        for row in rows
        if row["reached_objective"]
    )

    qualified_objective_count = sum(
        1
    ...
    print(
        f"Source sessions: "
        f"{args.source_sessions}"
    )

    print(
        f"Qualified sample size: "
        f"{args.sample_size}"
    )

    print(
        f"Source objective rate: "
        f"{round((objective_count / args.source_sessions) * 100, 6)} %"
    )

    print(
        f"Qualified objective rate: "
        f"{round((qualified_objective_count / args.sample_size) * 100, 6)} %"
    )

    print(
        f"Best final capital: "
        f"{qualified[0]['final_capital']} €"
    )

    print(
        f"Best session index: "
        f"{qualified[0]['session_index']}"
    )

    print(
        f"Qualified average final capital: "
        f"{round(mean(row['final_capital'] for row in qualified), 2)} €"
    )

    print(
        f"Qualified median final capital: "
        f"{round(median(row['final_capital'] for row in qualified), 2)} €"
    )
    ...
    for rank, row in enumerate(
        ...
        print(
            ...
            f"checkpoint={row['checkpoint_capital']} € | "
