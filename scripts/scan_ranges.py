...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Scan deterministic seed ranges for high-end trajectories"
    )
    ...
    print(
        f"Best range number: "
        f"{global_best_range + 1}"
    )

    print(
        f"Average final capital: "
        f"{round(mean(global_values), 2)} €"
    )

    print(
        f"Median final capital: "
        f"{round(median(global_values), 2)} €"
    )

    print(
        f"Objective rate: "
        f"{round((objective_count / total_sessions) * 100, 6)} %"
    )

    print(
        f"Capital floor rate: "
        f"{round((floor_count / total_sessions) * 100, 4)} %"
    )
