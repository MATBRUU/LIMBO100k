...
def parse_stages(raw: str) -> list[float]:
    ...
    if len(values) < 2:
        raise ValueError(
            "At least two stage values are required"
        )
...
def run_stage_attempt(
    ...
    return (
        ...
        or final_capital >= target_capital,
    )
...
def apply_lock_rule(
    ...
    if (
        reached_target < lock_from_stage
        or lock_ratio <= 0
    ):
...
def run_campaign(
    ...
    base_seed = (
        args.seed_offset
        + campaign_index * args.max_attempts
    )
    ...
    while (
        ...
        if hit:
            ...
            reserve += (
                max(0.0, final_capital - target)
                if args.return_excess
                else 0.0
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run staged bankroll campaigns"
    )
    ...
    parser.add_argument(
        "--stages",
        default="50,500,5000,25000,100000",
    )
    ...
    locked_campaigns = sum(
        1
        for row in rows
        if row["locked_total"] > 0
    )
    ...
    print(
        f"Average locked total: "
        f"{round(mean(locked_values), 2)} €"
    )

    print(
        f"Campaigns with locked capital: "
        f"{round((locked_campaigns / args.campaigns) * 100, 6)} %"
    )

    print(
        f"Success campaign rate: "
        f"{round((successes / args.campaigns) * 100, 6)} %"
    )

    print(
        f"Average stage depth: "
        f"{round(mean(stage_depths), 2)} / "
        f"{len(stages) - 1}"
    )

    print(
        f"Best campaign final: "
        f"{best_row['final_total']} €"
    )

    print(
        f"Best campaign index: "
        f"{best_row['campaign_index']}"
    )
    ...
    if args.export_csv:
        ...
        output_path = (
            output_dir
            / f"stage_campaign_{args.strategy}.csv"
        )
